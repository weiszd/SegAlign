    ...
    if(num_hsps > 0){
        ...
        if(fw_num_hsps > 0){
            ...
            if(cfg.gapped){
                ...
                cmd = "lastz "+cfg.data_folder+"ref.2bit[nameparse=darkspace][multiple][subset=ref_block"+std::to_string(r_block_index)+".name] "+cfg.data_folder+"query.2bit[nameparse=darkspace][subset=query_block"+std::to_string(q_block_index)+".name]"+
                    ...
                    " --ydrop="+std::to_string(cfg.ydrop)+
                    " --gappedthresh="+std::to_string(cfg.gappedthresh)+
                    " --strand=plus";

                if(cfg.inner != -1)
                    cmd += " --inner=" + std::to_string(cfg.inner);
                if(cfg.scoring_file != "")
                    cmd += " --scoring=" + cfg.scoring_file;
                if(cfg.gap != "")
                    cmd += " --gap=" + cfg.gap;                    
        ...
        if(rc_num_hsps > 0){
            ...
            if(cfg.gapped){
                ...
                //cmd = "lastz "+cfg.data_folder+"ref.2bit[nameparse=darkspace][multiple][subset=ref_block"+std::to_string(r_block_index)+".name] "+cfg.data_folder+"query.2bit[nameparse=darkspace][subset=query_block"+std::to_string(q_block_index)+".name] --format="+ cfg.output_format +" --ydrop="+std::to_string(cfg.ydrop)+" --gappedthresh="+std::to_string(cfg.gappedthresh)+" --strand=minus --inner="+std::to_string(cfg.inner);
                cmd = "lastz "+cfg.data_folder+"ref.2bit[nameparse=darkspace][multiple][subset=ref_block"+std::to_string(r_block_index)+".name] "+cfg.data_folder+"query.2bit[nameparse=darkspace][subset=query_block"+std::to_string(q_block_index)+".name]"+
                    " --format="+ cfg.output_format +
                    " --ydrop="+std::to_string(cfg.ydrop)+
                    " --gappedthresh="+std::to_string(cfg.gappedthresh)+
